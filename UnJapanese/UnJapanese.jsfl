/*
	...
	日本語でのデフォールトオブジェクト名を英訳して、
	全角英数字を半角に変更して、残りの見分けられない日本語を「JNAME」+数字に変更します。
	
	Copyright Joseph Jacir 4 June 2013
	v1.12
	
	WARNINGS: 		
		If you undo immediately after running this command, it will undo the renaming successfully, but it will also flatten your library's directory structure. This is the result of some ugliness in the way Flash handles directory names via script. It may be safer to save your file before running this command, and revert if necessary.
	注意
		このコマンドを実行して、すぐundoすれば、
		無事でコマンドを取り消しますが、
		それにライブラリーのディレクトリ構造を平らにしちゃう。
		フラッシュのJSFLでディレクトリー名をスクリプトで対応仕方のせいです。
*/
...
function strayJapanese(str) {
	...
	//	E.g. 何とか becomes JNAME0, or another number if 0 is already taken.
